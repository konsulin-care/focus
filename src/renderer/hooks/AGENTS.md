...
- Dependencies: Only import from `@/renderer/store`, `@/renderer/utils`, `@/renderer/types`, and allowed external modules like `react`, `react-dom` – avoid deep/internal imports to keep the hook reusable. (Deep/internal imports refer to deep paths within our repo; importing from external packages like react is permitted.)
