...
import { calculateVariability } from '@/shared/utils/basic-stats';
...
function manualInverseNormalCDF(probability: number): number {
  ...
  const numerator = 2.515517 + 0.802853 * sqrtTerm + 0.010328 * sqrtTerm * sqrtTerm;
 ...
 */
function manualCalculateDPrime(hitRate: number, falseAlarmRate: number): number {
  // Boundary adjustments
  const adjustedHitRate = hitRate <= 0 ? 0.00001 : hitRate >= 1 ? 0.99999 : hitRate;
  const adjustedFARate =
    falseAlarmRate <= 0 ? 0.00001 : falseAlarmRate >= 1 ? 0.99999 : falseAlarmRate;
...
describe('Attention Metrics - TOVA Manual Validation', () => {
  ...
  describe('calculateDPrime (Signal Detection)', () => {
    ...
    it('should match clinical-metrics calculateDPrime', () => {
      ...
      testCases.forEach(({ hitRate, faRate, description, expectedDPrime }) => {
        ...
        console.log(
          `[TEST] ${description || `HR=${hitRate}, FAR=${faRate}`}: manual=${manual.toFixed(4)}, library=${library.toFixed(4)}`
        );
    ...
    it('should calculate D Prime for test data second half', () => {
      ...
      const secondHalfOnsets = secondHalfEvents.filter(
        (e: TestEvent) => e.eventType === 'stimulus-onset'
      );
      const secondHalfResponses = secondHalfEvents.filter(
        (e: TestEvent) => e.eventType === 'response'
      );

      const secondHalfTargets = secondHalfOnsets.filter(
        (e: TestEvent) => e.stimulusType === 'target'
      ).length;
      const secondHalfNonTargets = secondHalfOnsets.filter(
        (e: TestEvent) => e.stimulusType === 'non-target'
      ).length;

      const secondHalfHits = secondHalfResponses.filter(
        (e: TestEvent) => e.responseCorrect === true
      ).length;
      const secondHalfCommissions = secondHalfResponses.filter(
        (e: TestEvent) => e.responseCorrect === false && e.stimulusType === 'non-target'
      ).length;

      const hitRate = secondHalfTargets > 0 ? secondHalfHits / secondHalfTargets : 0.5;
      const falseAlarmRate =
        secondHalfNonTargets > 0 ? secondHalfCommissions / secondHalfNonTargets : 0.5;
      ...
      console.log(
        '[TEST]   Non-Targets:',
        secondHalfNonTargets,
        'Commissions:',
        secondHalfCommissions
      );
  ...
  describe('Manual Calculation Validation - 32yo Male', () => {
    ...
    it('should calculate correct Z-scores using actual normative data', () => {
      ...
      console.log(
        '[TEST]   Normative DPrime Mean:',
        normativeStats.dPrimeMean,
        'SD:',
        normativeStats.dPrimeSD
      );
      console.log(
        '[TEST]   Normative RT Mean:',
        normativeStats.responseTimeMean,
        'SD:',
        normativeStats.responseTimeSD
      );
  ...
  describe('calculateAttentionMetrics (Integration)', () => {
    it('should return valid metrics for test data', () => {
      ...
      if (totalNonTargets > 0) {
        expect(metrics.commissionPercent).toBeCloseTo(
          (metrics.commissions / totalNonTargets) * 100,
          2
        );
      }
    });
    ...
    it('should calculate ACS with proper components', () => {
      ...
      expect(metrics.zScores.responseTime).toSatisfy(
        (v: number | null) => v === null || typeof v === 'number'
      );
      expect(metrics.zScores.dPrime).toSatisfy(
        (v: number | null) => v === null || typeof v === 'number'
      );
      expect(metrics.zScores.variability).toSatisfy(
        (v: number | null) => v === null || typeof v === 'number'
      );

      // Interpretation should be valid
      expect(['normal', 'borderline', 'not-within-normal-limits']).toContain(
    ...
    it('should match manual ACS calculation', () => {
      ...
      if (!normativeStats) {
        console.log(
          '[TEST] No normative data available for age:',
          subjectInfo.age,
          'gender:',
          subjectInfo.gender
        );
      ...
      const totalTrials = allEvents.filter(
        (e: TestEvent) => e.eventType === 'stimulus-onset'
      ).length;
      const midpoint = Math.floor(totalTrials / 2);
      ...
      const firstHalfMeanRT =
        firstHalfResponses.length > 0
          ? firstHalfResponses.reduce(
              (sum: number, e: TestEvent) => sum + (e.responseTimeMs as number),
              0
            ) / firstHalfResponses.length
          : metrics.meanResponseTimeMs;

      // Get overall valid hit response times
      const allValidHits = allEvents.filter(
        (e: TestEvent) =>
          e.eventType === 'response' &&
          e.responseCorrect === true &&
          !e.isAnticipatory &&
      ...
      const overallVariability =
        allValidHits.length > 0
          ? manualCalculateVariability(
              allValidHits.map((e: TestEvent) => e.responseTimeMs as number)
            )
          : metrics.variability;
      ...
      const manualACS =
        ...
        zScoreScaled(
          overallVariability,
          normativeStats.variabilityMean,
          normativeStats.variabilitySD,
          1
        ) +
      ...
      console.log(
        '[TEST] Normative Variability Mean:',
        normativeStats.variabilityMean,
        'SD:',
        normativeStats.variabilitySD
      );
  ...
  describe('Anticipatory Response Exclusion', () => {
    it('should exclude responses <150ms from variability calculation', () => {
      ...
      const anticipatoryResponses = events.filter(
        (e: TestEvent) => e.eventType === 'response' && e.isAnticipatory === true
      );

      // Count total responses to targets
      const allResponses = events.filter(
        (e: TestEvent) => e.eventType === 'response' && e.stimulusType === 'target'
      );
    ...
    it('should include only valid hits in mean RT calculation', () => {
      ...
      const validHits = events.filter(
        (e: TestEvent) =>
          e.eventType === 'response' && e.responseCorrect === true && !e.isAnticipatory
      );

      const anticipatoryHits = events.filter(
        (e: TestEvent) =>
          e.eventType === 'response' && e.responseCorrect === true && e.isAnticipatory === true
      );
