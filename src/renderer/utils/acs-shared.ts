...
import type { SubjectInfo, TrialResult } from '@/renderer/types/trial';
...
export function computeAcsValues(
  ...
  const firstHalfMeanRT =
    firstHalfResponseTimes.length > 0 ? calculateMean(firstHalfResponseTimes) : 0;
  ...
  const secondHalfCorrectRejections = secondHalfTrials.filter(
  ...
  const overallMeanRT =
    validHitResponseTimes.length > 0 ? calculateMean(validHitResponseTimes) : firstHalfMeanRT;

  const variability =
    validHitResponseTimes.length > 0
