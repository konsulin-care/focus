...
function calculateZScoresForDisplay(hitRate: number, falseAlarmRate: number) {
  ...
  const zFA =
    falseAlarmRate > 0.5 ? -getZScoreMagnitude(falseAlarmRate) : getZScoreMagnitude(falseAlarmRate);
  return { zHit, zFA };
}
...
function calculateDPrimeIntermediates(result: AcsIntermediateResult) {
  ...
  const secondHalfOmissions = result.secondHalfTrials.filter(
    (t) => t.outcome === 'omission'
  ).length;
  const secondHalfCommissions = result.secondHalfTrials.filter(
    (t) => t.outcome === 'commission'
  ).length;
  const secondHalfCorrectRejections = result.secondHalfTrials.filter(
    (t) => t.outcome === 'correct-rejection'
  ).length;
  ...
  return {
    ...
    zScores: {
      ...
      dPrime: {
        ...
        normMean:
          normativeAvailable && result.normativeStats ? result.normativeStats.dPrimeMean : null,
      ...
      variability: {
        ...
        normMean:
          normativeAvailable && result.normativeStats
            ? result.normativeStats.variabilityMean
            : null,
        normSD:
          normativeAvailable && result.normativeStats ? result.normativeStats.variabilitySD : null,
