...
- Architecture: the main process owns all auth logic (registration, login, session management, recovery) via `src/main/auth.ts`; cryptographic key material is managed by `src/main/key-management.ts`; `src/main/generated-config.ts` is auto-generated during CI builds and must never be committed.
...
- Detailed documentation is in `AUTH.md`.
